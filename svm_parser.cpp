...
const char* Token::token_names[25] = { "ID", "LABEL", "NUM", "EOL", "ERR", "END", "PUSH", "JMEPEQ", "JMPGT", "JMPGE", "JMPLT", "JMPLE", "GOTO", "SKIP", "POP", "DUP", "SWAP", "ADD", "SUB", "MUL", "DIV", "STORE", "LOAD", "PRINT", "FACT" };
...
Instruction* Parser::parseInstruction() {
  ...
  if (match(Token::POP) || match(Token::ADD) || match(Token::DUP) || match(Token::SWAP) ||
        match(Token::PRINT) || match(Token::SUB) || match(Token::MUL) || match(Token::DIV) ||
          match(Token::SKIP)) {  // mas casos
    //cout << "Type 0" << endl;
      tipo = 0;
    ttype = previous->type;
      ...
      ttype = previous->type;
      if(match(Token::NUM)){
          intvalue = stoi(previous->lexema);
      } else{
          cout << "Error: no pudo encontrar match para " << current << endl;
          exit(0);
      }

  } else if (match(Token::GOTO) || match(Token::JMPLE) || match(Token::JMPGT) || match(Token::JMPGE)
                || match(Token::JMPLT) || match(Token::JMPEQ)) { // mas casos
      //cout << "Type 2" << endl;
      tipo = 2;
    ttype = previous->type;
    if(match(Token::ID) || match(Token::LABEL)){
  ...
  if (tipo == 0) {
      //cout << "Creating instruction for type 0" << endl;
    instr = new Instruction(label, Token::tokenToIType(ttype));
  } else if (tipo == 2) {
      //cout << "Creating instruction for type 2" << endl;
    instr = new Instruction(label, Token::tokenToIType(ttype), jmplabel);
  } else { //
      //cout << "Creating instruction for type 1" << endl;
      instr = new Instruction(label, Token::tokenToIType(ttype), intvalue);
  }
